...
public class Graph {
  ...
  private int paneSideLenght;// to be changed if necessary
  private int boundaryStart; // This denotes the leftmost or topmost pixel of the area within which we want the graph to appear
  private int boundaryEnd;// This denotes the rightmost or downmost pixel of the area within which we want the graph to appear
  private final int DIAMETER = 16;

  public Graph (int[][] ColEdge, int paneSideLenght, int boundaryStart, int boundaryEnd) {

    this.paneSideLenght = paneSideLenght;
    ...
    for (int i=1; i<=vertices.size(); i++) {
      ...
      while (vertexOverlapFound(vertex, vertices) || notWithinBounds(vertex)) {
         vertex.setMidX((int) (Math.random()*paneSideLenght));
         vertex.setMidY((int) (Math.random()*paneSideLenght));
       }
     }
...
// Method for getting the list of vertices as a HashMap
  ...
  public boolean notWithinBounds(Vertex vertex) {
    ...
    if (vertex.getMidX()<boundaryStart ||
        vertex.getMidX()>boundaryEnd ||
        vertex.getMidY()<boundaryStart ||
        vertex.getMidY()>boundaryEnd) {
